...
public class DBActions extends SQLiteOpenHelper implements DBComunication{
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String createTable = "CREATE TABLE " + TasksTable.TABLE + " (" + TasksTable._ID + " INTEGER PRIMARY KEY AUTOINCREMENT," +
                TasksTable.COL_TASK_DONE + " INTEGER DEFAULT 0, " + TasksTable.COL_TASK_TITLE + " TEXT NOT NULL" + ");";
    ...
    public ArrayList<ItemVO> getAllItems() {
        ...
        while (cursor.moveToNext()) {
            int idInex = cursor.getColumnIndex(TasksTable._ID);
            ...
            itemVO.id = cursor.getInt(idInex);
    ...
    public void deleteItem(ItemVO itemVO) {
        ...
        db.delete(TasksTable.TABLE, TasksTable._ID + "=?", new String[]
                {Integer.toString(itemVO.id)});
        db.close();
    ...
    SQLiteDatabase db = this.getWritableDatabase();
    db.updateWithOnConflict(TasksTable.TABLE, contentValues, TasksTable._ID + "=?",
            new String[]{Integer.toString(itemVO.id)}, SQLiteDatabase.CONFLICT_REPLACE);
    db.close();
...
class TasksTable implements BaseColumns {
    public static final String TABLE = "taskas";
